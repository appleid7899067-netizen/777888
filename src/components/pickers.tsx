      ...
      <SelectTrigger className={compact ? "h-8 w-[min(100%,16rem)] text-xs" : "w-[min(100%,22rem)]"}>
        <SelectValue>
          <span className="flex items-center gap-2 truncate">
            ...
            <span className="text-subtle">{model.credits} cr</span>
          </span>
        </SelectValue>
      </SelectTrigger>
      <SelectContent>
        {(["free", "standard", "premium", "elite", "ultimate"] as FleetModel["tier"][]).map((tier) => {
              ...
              {group.map((m) => (
                <SelectItem key={m.id} value={m.id}>
                  <span className="flex w-full items-center justify-between gap-3">
                    ...
                    <Badge variant={m.credits === 0 ? "ok" : "default"}>{m.credits} cr</Badge>
                  </span>
                </SelectItem>
              ))}
            </SelectGroup>
