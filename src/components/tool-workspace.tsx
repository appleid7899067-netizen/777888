...
import { modelById, type FleetTool } from "@/lib/catalog";
    ...
    <AppShell>
      <div className="mx-auto grid max-w-6xl gap-6 px-4 py-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <section className="min-w-0">
          <p className="text-[11px] font-medium uppercase tracking-wider text-subtle">Tool</p>
          ...
          {tool.kind !== "diagram" && (
            <div className="mt-3 space-y-1.5">
              <div className="flex items-center justify-between">
                <Label htmlFor="code">
                  {tool.kind === "diagram-to-code" ? "Diagram source" : "Code"}
                </Label>
                <button
                  type="button"
          ...
          <label className="mt-3 flex cursor-pointer items-center gap-3 rounded-lg bg-elevated px-3 py-3 text-sm text-muted shadow-[var(--shadow-border)]">
            ...
            <span>Drop files or a project zip — stored in this browser only.</span>
            <input
              type="file"
              multiple
              className="hidden"
          ...
          {files.length > 0 && (
            <p className="mt-2 text-xs text-subtle">{files.join(" · ")}</p>
          )}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <Button onClick={generate} disabled={busy} className="min-w-36">
              {busy ? <Loader2 className="size-4 animate-spin" /> : <Sparkles className="size-4" />}
              {busy ? "Working" : tool.cta}
            </Button>
            <span className="text-xs text-subtle tabular-nums">Cost: {cost} credit{cost === 1 ? "" : "s"}</span>
          </div>
        ...
        <section className="min-w-0">
          <div className="flex h-full min-h-80 flex-col rounded-xl bg-surface p-4 shadow-[var(--shadow-border)] lg:p-5">
            ...
            {busy && (
              <div className="fleet-shimmer h-1 rounded-full" />
            )}
            {output ? (
              ...
              <div className="flex flex-1 items-center justify-center text-center text-sm text-subtle">
                {busy ? "The fleet is writing…" : "Output lands here. Nothing leaves this browser except the prompt."}
              </div>
            )}
          </div>
        </section>
