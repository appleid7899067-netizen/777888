  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-6xl px-4 py-8">
        <p className="text-[11px] font-medium uppercase tracking-wider text-subtle">Automation</p>
        <h1 className="mt-1 text-3xl font-medium tracking-tight">Routines</h1>
        <p className="mt-2 max-w-2xl text-sm text-muted">
          Saved prompts on a cadence. They can touch MCP connectors and a linked repo. Run now, or let them fire later.
        </p>
        ...
        <ul className="mt-6 space-y-3">
          {routines.map((r) => (
            <li key={r.id} className="flex flex-col gap-3 rounded-xl bg-surface p-4 shadow-[var(--shadow-border)] sm:flex-row sm:items-center">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <h2 className="font-medium">{r.name}</h2>
                  <Badge variant={r.enabled ? "ok" : "default"}>{r.enabled ? "on" : "paused"}</Badge>
                </div>
                <p className="text-xs text-subtle">{r.cadence}</p>
                <p className="mt-1 truncate text-sm text-muted">{r.prompt}</p>
                {r.lastRun ? (
