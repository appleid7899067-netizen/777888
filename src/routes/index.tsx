...
import { STATS, TOOLS, AGENTS } from "@/lib/catalog";
    ...
    <AppShell marketing>
      <section className="relative overflow-hidden">
        <div
          ...
          style={{
            background:
              "radial-gradient(800px 400px at 80% -10%, color-mix(in oklab, var(--color-primary) 14%, transparent), transparent 70%)",
          }}
        />
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-4 py-14 lg:grid-cols-[1.1fr_0.9fr] lg:py-20">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-elevated px-3 py-1 text-xs text-muted shadow-[var(--shadow-border)]">
              ...
              20+ LLMs · Parallel agents · Code exec
            </div>
            <h1 className="mt-5 text-4xl font-medium tracking-tight sm:text-5xl">
              AI-powered coding tools.
              <span className="mt-2 block text-muted">A fleet that cooperates.</span>
            </h1>
            <p className="mt-5 max-w-prose text-base text-muted">
              Generate, refactor, test, convert, and ship. One workspace for models, sandboxes, and
              agents that split the work instead of taking turns at a single prompt.
            </p>
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <Button size="lg" variant="secondary" asChild>
                <Link to="/agents">Open the fleet</Link>
              </Button>
            </div>
            <dl className="mt-10 grid max-w-lg grid-cols-3 gap-4">
              <Stat n={formatNumber(STATS.generations)} l="Generations" />
              <Stat n={formatNumber(STATS.programmers)} l="Programmers" />
              <Stat n={`${STATS.languages}+`} l="Languages" />
            </dl>
          </div>
          ...
          <div className="min-w-0 rounded-xl bg-surface p-3 shadow-[var(--shadow-border)]">
            ...
            <div className="mt-3 grid gap-2">
              ...
              <CodePane title="Fleet" body={current.to} accent />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-border">
        <div className="mx-auto max-w-6xl px-4 py-14">
          ...
          <p className="mt-2 max-w-prose text-sm text-muted">
            Each tool is a focused workflow — not a chatbot wearing a different hat. Chat is there when you need it.
          </p>
      ...
      <section className="border-t border-border bg-surface">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 py-14 lg:grid-cols-2">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-medium tracking-tight">One chat. A fleet that cooperates.</h2>
            <p className="mt-3 text-sm text-muted">
              Mention @shipper, @security, @reviewer. The orchestrator frames bounded sub-tasks, runs
              read-only agents in parallel, serializes writes, and brings one synthesis back to the thread.
            </p>
            <ul className="mt-5 space-y-3 text-sm text-muted">
              ...
              <Feature icon={Terminal} text="Sandboxed code execution with snapshots." />
            </ul>
      ...
      <section className="border-t border-border">
        <div className="mx-auto max-w-6xl px-4 py-14">
          <p className="text-[11px] font-medium uppercase tracking-wider text-subtle">How it works</p>
          <h2 className="mt-2 text-2xl font-medium tracking-tight">From curiosity to shipping — four steps.</h2>
          <ol className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { n: "01", t: "Explore", d: "Use any tool. Guest credits are enough to feel the fleet." },
              { n: "02", t: "Pick a model", d: "Auto-Select, Grok, or a specialist. Cost is shown before you send." },
              { n: "03", t: "Run it", d: "Sandbox, tests, diagrams, review. Advanced tools when you need them." },
              { n: "04", t: "Ship", d: "Copy, download, or hand the patch to @shipper for a durable change." },
            ].map((s) => (
      ...
      <section className="border-t border-border">
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-6 px-4 py-14 sm:flex-row sm:items-center">
          <div>
            <h2 className="text-2xl font-medium tracking-tight">Ready when you are.</h2>
            <p className="mt-2 text-sm text-muted">
              No account required in this prototype. Credits live in your browser.
            </p>
          </div>
...
function CodePane({ title, body, accent }: { title: string; body: string; accent?: boolean }) {
  return (
    <div className="min-w-0 overflow-hidden rounded-lg bg-bg shadow-[var(--shadow-border)]">
      <p className="border-b border-border px-3 py-1.5 text-[11px] uppercase tracking-wider text-subtle">
        {title}
      </p>
      <pre
