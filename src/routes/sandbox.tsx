    ...
    <AppShell>
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted">
          Isolated runtimes reused across chats. Packages, snapshots, and network isolation stay with the box.
        </p>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          {sandboxes.map((s) => (
            <article key={s.id} className="rounded-xl bg-surface p-5 shadow-[var(--shadow-border)]">
              ...
              <p className="mt-1 text-xs text-subtle">{s.snapshots} snapshot{s.snapshots === 1 ? "" : "s"}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <Button size="sm" variant="secondary" onClick={() => snapshotSandbox(s.id)}>
